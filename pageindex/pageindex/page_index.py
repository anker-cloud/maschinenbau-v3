...
def toc_transformer(toc_content, model=None):
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        # If the LLM finished normally and the content is valid JSON, trust it
        if finish_reason == "finished":
            try:
                parsed = json.loads(last_complete)
                if 'table_of_contents' in parsed:
                    if_complete = "yes"
            except Exception:
                pass
...
def page_index_main(doc, opt=None):
    _utils._LLM_SEMAPHORE = None  # Reset semaphore — asyncio.run() creates a fresh event loop each call
