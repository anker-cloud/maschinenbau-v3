...
_MAX_RETRIES = int(os.environ.get("PAGEINDEX_MAX_RETRIES", "3"))
_BASE_BACKOFF = float(os.environ.get("PAGEINDEX_BACKOFF_BASE", "0.5"))


def _backoff(i: int) -> float:
    # Exponential backoff with jitter, capped at 8s.
    import random
    return min(8.0, _BASE_BACKOFF * (2 ** i)) + random.uniform(0, 0.25)
    ...
    for i in range(_MAX_RETRIES):
        try:
            ...
            if i < _MAX_RETRIES - 1:
                ...
                logging.error('PageIndex LLM: max retries reached')
                if return_finish_reason:
                    return "", "error"
                return ""
